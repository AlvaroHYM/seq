...
def log_training(trainer, name, description):
	"""
...
class MetricWatcher:
	...
	"""
	def __init__(self, metric, monitor, mode, base=None):
		self.best = base
		self.metric = metric
		self.mode = mode
	...
	def has_improved(self):
		...
		if not self.best or math.isnan(self.best):
			...
			return True
		if self.mode == "min" and last_metric_val < self.best:
			self.best = last_metric_val
			return True
		if self.mode == "max" and last_metric_val > self.best:
			self.best = last_metric_val
			return True
